...
class Store {
  ...
  getUserStorageKey() {
    const user = api.getCurrentUser();
    return user ? `campushub_state_user_${user.id}` : STORAGE_KEY;
  }
  ...
  async setCurrentUser(user) {
    if (user) {
      ...
      this.state.currentRole = user.role || 'Student';
      try {
        const remoteState = await api.getUserData('full_state');
        if (remoteState) {
          this.state = { ...initialData, ...remoteState, profile: { ...this.state.profile, ...user } };
        } else {
          const localSaved = localStorage.getItem(this.getUserStorageKey());
          if (localSaved) {
            this.state = { ...initialData, ...JSON.parse(localSaved), profile: { ...this.state.profile, ...user } };
          }
        }
      } catch (e) {
        console.warn('Could not load remote state', e);
      }
      this.saveState();
      this.emit('auth:updated', user);
      this.emit('profile:updated', this.state.profile);
      this.emit('*', { event: 'user_switched' });
    }
  }
  ...
  logout() {
    ...
    this.emit('profile:updated', this.state.profile);
    this.emit('*', { event: 'user_logged_out' });
  }
  ...
  saveState(stateToSave = this.state) {
    ...
    try {
      ...
      // Asynchronously sync to SQLite backend
      api.saveUserData('full_state', stateToSave).catch(() => {});
    } catch (err) {
